...
sabre["ShaderPrototype"] = Object.create(Object,{
	...
	'load':{
		value: function(vertexUrl,fragmentUrl,expire){
			if(typeof(global.localStorage) === "undefined"||typeof(expire) === 'undefined'||expire <= 0){
			...
			if(window.localStorage.getItem(storageNameVertex)=== null ||parseInt(window.localStorage.getItem(storageNameVertex).split('\u0003')[1],16)<window.Date.now()){
				var xmlhttp = new XMLHttpRequest();
				xmlhttp.open("GET", vertexUrl,false);
				xmlhttp.overrideMimeType("text/plain");
				xmlhttp.send();
				if ((xmlhttp.status === 200))
				{
						window.localStorage.setItem(storageNameVertex,xmlhttp.responseText+"\u0003"+((expire*86400000)+window.Date.now()).toString(16));
						shaderlog[vertexUrl] = xmlhttp.responseText;
						this.vertSrc = xmlhttp.responseText;
				}
			}else{
			...
			if(window.localStorage.getItem(storageNameFragment)=== null||parseInt(window.localStorage.getItem(storageNameFragment).split('\u0003')[1],16)<window.Date.now()){
				var xmlhttp = new XMLHttpRequest();
				xmlhttp.open("GET", fragmentUrl,false);
				xmlhttp.overrideMimeType("text/plain");
				xmlhttp.send();
				if ((xmlhttp.status === 200))
				{
						window.localStorage.setItem(storageNameFragment,xmlhttp.responseText+"\u0003"+((expire*86400000)+window.Date.now()).toString(16));
						shaderlog[fragmentUrl] = xmlhttp.responseText;
						this.fragSrc = xmlhttp.responseText;
				}
			}else{
	...
	'compile': {
		value: function(gl,defines,err,version) {
			...
			this._vert = this._compile(gl,this.vertSrc,defines,gl.VERTEX_SHADER,version);
			this._frag = this._compile(gl,this.fragSrc,defines,gl.FRAGMENT_SHADER,version);
			...
			if(defines != null){
				...
				for(var i = 0; i < define_names.length;i++){
					if(defines[define_names[i]] == true)
						...
						shaderHeader += "#define " + define_names[i] + " " + defines[define_names[i]].toString() + "\n";
				}
			}
