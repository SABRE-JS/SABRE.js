...
(function(global,exterior){
   ...
   */
	const Complaint = function(warning){
		...
		var newComplaint = global.Object.seal(global.Object.create({_issue:warning,_grumbled:false},ComplaintPrototype));
   ...
   */
	Complaint["resetAll"] = function(){
		allComplaints.forEach(function(val){val["reset"]();});
	};
	exterior.Complaint = global.Object.freeze(Complaint);
})(global,sabre);
//END ONE TIME WARN CODE

global.Math.trunc = global.Math.trunc||function(a){return a<0?Math.ceil(a):Math.floor(a);};
...
const canvas2blob = function (callback, type, quality) {
  ...
  var header = 'BM' + 
  lehex((imgdata.data.length||4*this.width*this.height)+14+108)+
  '\x00\x00' +
  '\x00\x00' + 
  '\x7A\x00\x00\x00' +
  '\x6C\x00\x00\x00' +
  ...
  '\x00\x00\x00\x00' +
  '\x00\x00\x00\x00';
  var arr = new ArrayBuffer((imgdata.data.length||4*this.width*this.height) + header.length + 4 - (header.length % 4));
  ...
  for (var j = 0; j < (imgdata.width||this.width); j++) {
    for (var l = 0; l < (imgdata.height||this.height); l++) {
      ...
      var n = ((imgdata.width||this.width) * ((imgdata.height||this.height) - l)) - ((imgdata.width||this.width) - j) + k;
      if(imgdata.data.length == 0){
        longs[n] = 0x00000000;
        continue;
      }
      longs[n] = (imgdata.data[(i*4)+1] << 24)
...
global.HTMLCanvasElement.prototype.toBlob = global.HTMLCanvasElement.prototype.toBlob||canvas2blob;
global.HTMLCanvasElement.prototype.toBlobHD = global.HTMLCanvasElement.prototype.toBlobHD||global.HTMLCanvasElement.prototype.toBlob;
